        ...
        char** argv)
{
    ...
    for (int i = 0; i < parse.optionsCount(); ++i)
    {
        ...
        switch (opt.index())
        {
            ...
            case optionIndex::TCP_ADDRESS:
                tcp_address = std::string(opt.arg);
                break;

            case optionIndex::TCP_PORT:
                tcp_port = static_cast<uint16_t>(strtol(opt.arg, nullptr, 10));
                break;
    ...
    switch (type)
    {
        case PUBLISHER:
        {
            ...
            if (mypub.init(topic_name, static_cast<uint32_t>(domain), static_cast<uint32_t>(num_wait_matched), async,
                    ...
                    participant_profile, tcp_address, tcp_port))
            {
        ...
        case SUBSCRIBER:
        {
            ...
            if (mysub.init(topic_name, static_cast<uint32_t>(count), static_cast<uint32_t>(domain), transport,
                    reliable, transient, hops, partitions, use_ownership, participant_profile, tcp_address, tcp_port))
            {
