        ...
        if (transport != DEFAULT || hops > 0 )
        {
            ...
            switch ( transport )
            {
                ...
                case TCPv4:
                {
                    ...
                    eprosima::fastrtps::rtps::IPLocator::setIPv4(initial_peer_locator, tcp_address);
                    eprosima::fastrtps::rtps::IPLocator::setPhysicalPort(initial_peer_locator, tcp_port);
        ...
        DataReader* reader)
{
    ...
    while ((reader->take_next_sample(&hello_, &info) == ReturnCode_t::RETCODE_OK) && !is_stopped())
    {
        if (info.instance_state == ALIVE_INSTANCE_STATE)
        {
            ...
            auto send_time_ns = info.reception_timestamp.to_ns() - info.source_timestamp.to_ns();
            auto send_time_s = send_time_ns / 1e9;
            std::cout << "Send time: " << send_time_s << " seconds" << std::endl;
