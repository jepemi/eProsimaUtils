...
struct Arg : public option::Arg
{
            ...
            bool msg)
    {
        if (option.arg != 0)
        {
            ...
            if (transport != "shm" && transport != "udp" && transport != "udpv4" && transport != "udpv6" && transport != "tcp")
            {
                if (msg)
                {
                    print_error("Option '", option, "' only accepts <shm|udp[v4]|udpv6|tcp> values\n");
                }
                return option::ARG_ILLEGAL;
            }
            return option::ARG_OK;
...
const option::Descriptor usage[] = {
    ...
    { TRANSPORT, 0, "", "transport",        Arg::Transport,
      "  \t--transport=<shm|udp|udpv6|tcp> \tUse only shared-memory, UDPv4, UDPv6 ot TCPv4 transport."
      "If not set, use Fast DDS default transports (depending on the scenario it will use the most efficient one:"
      " data-sharing delivery mechanism > shared-memory > UDP)." },
    { OWNERSHIP, 0, "o", "ownership",        Arg::None,
      "  -o \t--ownership \tUse Topic with EXCLUSIVE_OWNERSHIP (SHARED_OWNERSHIP by default)."},
    ...
    { UNKNOWN_OPT, 0, "", "",                Arg::None,      "\nTCP options:"},
    { TCP_ADDRESS, 0, "", "tcp-address",         Arg::String,
      "\t--tcp-address \tSet the IP address to bind the TCP transport. If not set, uses localhost."},
    { TCP_PORT, 0, "", "tcp-port",         Arg::NumericRange<1, 65535>,
      "\t--tcp-port \tSet the port to bind the TCP transport. If not set, 5100."},
